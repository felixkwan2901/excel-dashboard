...
export default function SiteQrCode({ jobNumber, jobName }) {
  ...
  // Flags the page so the print stylesheet can isolate this card. Without it
  // `window.print()` prints the whole job page — the sheet that goes up on a
  // switchboard door would carry the claim percentages, projected cost and
  // margin that happened to be on screen behind it.
  const printCode = () => {
    const root = document.documentElement
    root.setAttribute('data-printing', 'qr')
    window.addEventListener('afterprint', () => root.removeAttribute('data-printing'), {
      once: true,
    })
    window.print()
  }
  ...
  return (
    <div className="site-qr mt-6 border-t border-white/[0.06] pt-5">
      <div className="site-qr__row flex flex-wrap items-start gap-5">
        {/* Always on white, in both themes. A dark-mode QR with a dark
            "quiet zone" is unreadable to a phone camera, and it would print
            as a black square. */}
        <div className="site-qr__plate shrink-0 rounded-xl bg-white p-2.5">
          {svg ? (
            <div
              className="site-qr__code h-[132px] w-[132px] [&>svg]:h-full [&>svg]:w-full"
              dangerouslySetInnerHTML={{ __html: svg }}
            />
          ) : (
            <div className="site-qr__code h-[132px] w-[132px] animate-pulse rounded bg-neutral-200" />
          )}
        </div>

        <div className="site-qr__body min-w-0 flex-1">
          {/* The job's identity is the one thing the printed sheet has to
              carry: a bare code on a door tells nobody which job it opens.
              Number and name only — no costs, no client details. */}
          <p className="site-qr__print-title hidden">
            {jobNumber} {jobName}
          </p>
          <h3 className="no-print text-[14px] font-medium text-neutral-100">Site code</h3>
          <p className="no-print mt-1 text-[13px] text-neutral-400">
          ...
          <p className="site-qr__url mt-2 break-all font-mono text-[11px] text-neutral-500">{url}</p>
          <button
            onClick={printCode}
