#!/usr/bin/env node
// Applies staged Claim Calculator edits from pending-updates/claim-calculator/
// — run by .github/workflows/process-pending-updates.yml after the
// upload-worker's /claim-calculator endpoint stages each edit there.
//
...
const STAGING_DIR = resolve('pending-updates/claim-calculator')
...
// Only these columns may ever be written here — everything else on this
...
async function main() {
  if (!existsSync(STAGING_DIR)) {
    console.log('No pending Claim Calculator edits.')
    return
  }
  ...
  const ws = wb.getWorksheet('Claim Calculator By Month')
  if (!ws) throw new Error('Could not find "Claim Calculator By Month" in the current workbook')

  // Job Number (col A) is a formula (`='Main Sheet'!A<n>'`) on this sheet,
  // not a plain value — match by its cached result, same approach the
  // checklist's apply script uses for Main Sheet's own (plain-value) job
  // numbers.
  ...
  for (const name of files) {
    ...
    for (const edit of edits) {
      ...
      const value = col === 16 ? String(edit.value ?? '') : Number(edit.value)
      if (col !== 16 && !Number.isFinite(value)) {
